module EasyTable::ViewExt::Table
  module Data
    def data_table(collection, headers, options = {})    
      obj_type = Helper.get_obj_type collection
      Helper.set_options! options

      return options[:placeholder] unless collection.any?

      # try to extract attributes from headers!
      attributes = options[:attributes] || Helper.extract_attributes(collection.first, headers)
    ...
    module Helper
      def self.extract_attributes obj, headers
      ...
      def self.set_options! options={}, obj_type=nil
      ...
      def self.get_obj_type collection
